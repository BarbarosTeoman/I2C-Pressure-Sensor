...
{
    CyRamVectors[CYINT_IRQ_BASE + BUTTON_ISR__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + BUTTON_ISR__INTC_NUMBER];
}
...
void BUTTON_ISR_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((BUTTON_ISR__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *BUTTON_ISR_INTC_PRIOR = (*BUTTON_ISR_INTC_PRIOR & (uint32)(~BUTTON_ISR__INTC_PRIOR_MASK)) |
...
uint8 BUTTON_ISR_GetPriority(void)
{
    ...
    priority = (*BUTTON_ISR_INTC_PRIOR & BUTTON_ISR__INTC_PRIOR_MASK) >> priorityOffset;
